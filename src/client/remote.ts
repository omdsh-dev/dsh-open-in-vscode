...
declare module '@deepseek-ai/dsh-typert-protocol' {
  ...
  interface TypertRemoteNamespace$6f70656e496e5673636f6465 {
    ...
    openInExplorer: (path: string, signal?: AbortSignal) => Promise<RemoteResult<{ opened: true }>>
    openInPowerShell: (path: string, signal?: AbortSignal) => Promise<RemoteResult<{ opened: true }>>
  }
  interface TypertRemoteMap {
    'openInVscode/open': (path: string, signal?: AbortSignal) => Promise<RemoteResult<{ opened: true }>>
    'openInVscode/openInExplorer': (path: string, signal?: AbortSignal) => Promise<RemoteResult<{ opened: true }>>
    'openInVscode/openInPowerShell': (path: string, signal?: AbortSignal) => Promise<RemoteResult<{ opened: true }>>
  }
