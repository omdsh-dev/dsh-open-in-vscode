/**
 * Wire-contract invariants: the three strict endpoints (editor, Explorer,
 * PowerShell), one descriptor set shared verbatim by the host manifest and
 * the client contribution, and boundary codecs that parse and reject their
 * values.
 */
...
describe('the openInVscode wire contract', () => {
  it('declares the three strict endpoints shared by host and client', () => {
    expect(OPEN_IN_VSCODE_INVOCATIONS.map(invocation => invocation.method)).toEqual([
      'open',
      'openInExplorer',
      'openInPowerShell',
    ])
    for (const invocation of OPEN_IN_VSCODE_INVOCATIONS) {
      ...
      expect(invocation.id).toBe(`dsh-open-in-vscode#openInVscode/${invocation.method}`)
