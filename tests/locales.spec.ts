...
describe('the open-in-vscode dictionaries', () => {
  ...
  it('registers the expected namespace', () => {
    ...
    expect(zh['menu.openInExplorer']).toBe('在资源管理器中打开')
    expect(en['menu.openInExplorer']).toBe('Open in Explorer')
    expect(zh['menu.openInPowerShell']).toBe('在 PowerShell 中打开')
    expect(en['menu.openInPowerShell']).toBe('Open in PowerShell')
  })
})
